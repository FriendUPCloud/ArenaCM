...
class cSMS
{
	...
	var $_password;
	var $_host;
	var $_supportmail = 'hogne@ideverket.no';
...
";
		...
		// Debug
		if ( $output && substr( $output, 0, 5 ) == "<?xml" )
		{
			$debug = simplexml_load_string ( trim ( $output ) );
			
			if ( $this->_supportmail && isset( $debug->sendMsgResp->fault ) )
			{
				mail( $this->_supportmail , 'Clickatell Error', $log );
			}
		}
	}
...
<kqrepsms>
	...
	//#### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### 
	//#### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### 
	// Sending with linkmobility // pswincom
	// added by Thomas Wollburg 2015-10-02
	
	function send_pswincom()
	{
		if ( !count ( $this->_receivers ) )
			return false;
		...
		$user = $this->_username;
		$pass = $this->_password;
		// not needed $api_id = $this->_api_id;
		...
		foreach ( $this->_receivers as $recv )
		{
			...
			$r_str .= '<MSG><ID>'.$rcount.'</ID><RCV>' . $recv . '</RCV><SND>' . $this->_fromName . '</SND><TEXT>'. $this->_message .'</TEXT></MSG>';
			$rcount++;
		}
		
		// Generate xml as defined per https://wiki.pswin.com/Gateway%20XML%20API.ashx 
		$xml = "<?xml version=\"1.0\" ?>
<SESSION>
  <CLIENT>{$user}</CLIENT>
  <PW>{$pass}</PW>
  <MSGLST>
    {$r_str}
		...
		// Try to log
		if ( $f = fopen ( 'upload/sms_pswincom.log', 'a+' ) )
		{
			fwrite ( $f, "
SMS Output log: " . date ( 'Y-m-d H:i:s' ) . "
